...
Architecture:
    RGB ──→ 2D Visual Encoder (SigLIP) ──→ 2D Tokens ─┐
                                                        ├─→ Fusion ──→ LLM ──→ Constraint
    RGBD ──→ DepthToGaussian ──→ GS Encoder ──→ 3D Tokens ─┘
    Text ──→ Tokenizer ──────────────────────────────────→ LLM

Affordance UV is no longer a regression head — V2 plans emit textual
affordance_region descriptions, and runtime grounding (Lang-SAM) resolves
them to image coordinates downstream.
"""
