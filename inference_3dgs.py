...
_TASK_PROMPTS = {
    ...
    "planning": (
        # IMPORTANT: this few-shot mirrors the V2 training target format produced
        # by data/unified_loader.py:_load_plan_target() byte-for-byte. Any
        # divergence (re-introducing [u, v] coords, splitting approach onto its
        # own line, dropping <think>/Goal/<END_OF_PLAN>) forces the LoRA-trained
        # model into format collapse. Edit with care.
        'Plan the manipulation steps to complete the task: "{text}". Choose '
        'operation primitives from {{reach, grasp, transport, place, push, '
        'pull, insert, pour, rotate, release, flip, wipe}}.\n\n'
        'Output the plan in EXACTLY this compact text format. No JSON, no '
        'markdown, no bold. Field layout:\n'
        '<think>SCENE ANALYSIS - ... STRATEGY - ... AFFORDANCE REASONING - ...</think>\n'
        'Goal: <pred>(<args>); <pred>(<args>); ...\n'
        'Scene: <obj1>, <obj2>, ...\n'
        'Step <N>: <action>(<target>)             '
        '# use <action>(<target> -> <destination>) for transport/place/insert/pour\n'
        '  affordance_region: <specific, visually distinctive part description>\n'
        ...
        '<END_OF_PLAN>\n\n'
        'Requirements:\n'
        '  - The reasoning MUST be wrapped in literal "<think>" and "</think>" '
        'XML-style tags (the first emitted character is "<think>"). The '
        'tags are part of the output, not a placeholder. The block must '
        'causally derive the plan (not rationalize it). Cover visible '
        'objects, ordering, failure modes.\n'
        '  - Goal predicates are the sole termination contract — use lifted '
        'set-level predicates (stacked(S), all_inside(S, container), '
        'all_on(S, surface), any_in) when multiple permutations satisfy the '
        'task; grounded predicates (on(A, B), inside(A, B)) only for singleton '
        'goals.\n'
        '  - affordance_region is a textual description (a downstream '
        'grounding model resolves it to pixel coords at runtime). Never invent '
        'a part that is not in the image.\n'
        '  - Approach direction belongs inside the direction: category '
        '(e.g. "direction: approach_axis(0, 0, -1)"), not as a separate line.\n'
        '  - The plan ends with the literal token <END_OF_PLAN> on its own line.\n\n'
        'Example (follow the FORMAT, not the values):\n'
        '<think>SCENE ANALYSIS - A red mug stands on a wooden table next to a '
        'blue plate. The mug is rigid with a side handle; the plate is flat. '
        'STRATEGY - reach the mug at its handle for stable pinch grip, lift '
        'and transport above the plate, place gently to avoid sliding. '
        'AFFORDANCE REASONING - Step 1 reach mug body; Step 2 grasp handle for '
        'secure hold; Step 3 transport above plate; Step 4 lower onto plate '
        'center.</think>\n'
        ...
        '  affordance_region: the side handle of the red mug\n'
        '  contact: holding(red_mug) [completion]\n'
        '  direction: grasp_axis(0, 0, -1) [progress]\n'
        '  done_when: holding(red_mug)\n'
        'Step 3: transport(red_mug -> blue_plate)\n'
        '  affordance_region: an empty area on the blue plate\n'
        '  spatial: above(red_mug, blue_plate, 0.05) [completion]\n'
        '  safety: no_drop(red_mug)\n'
        '  done_when: above(red_mug, blue_plate, 0.05)\n'
        'Step 4: place(red_mug -> blue_plate)\n'
        '  affordance_region: an empty area on the blue plate\n'
        '  contact: surface_contact(red_mug, blue_plate) [completion]; '
        'released(red_mug) [completion]\n'
        '  pose: stable(red_mug) [completion]\n'
        '  done_when: surface_contact(red_mug, blue_plate) AND released(red_mug)\n'
        '<END_OF_PLAN>\n\n'
        'Now produce the plan for the actual scene. Decompose the task into '
        'ALL the steps needed to fully complete it (do not stop after one '
        'pick-and-place if the task involves multiple objects). End with '
        '<END_OF_PLAN>.'
    ),
    "general": "{text}",
}
...
class UnifiedInference3DGS:
    ...
    # ------------------------------------------------------------------
    # Two-stage inference: LoRA plan + base-model pointing refine
    # ------------------------------------------------------------------

    @torch.no_grad()
    def refine_affordance_via_base(
        self,
        plan: dict,
        image_path: str,
        ...
        with adapter_ctx:
            for step in steps:
                # V2 emits affordance_region; V1 emits affordance_hint. Use
                # whichever is present (region wins).
                hint = step.get("affordance_region") or step.get("affordance_hint")
