...
class UnifiedDataset(Dataset):
    ...
    def _load_plan_target(self, ep_dir: Path) -> str:
        """Load V2 GPT-generated plan as the training target.

        V2 format:
          - top-level `reasoning` → wrapped in <think>...</think>
          - top-level `goal` (list of predicates) → "Goal:" line, sole termination contract
          - scene_objects → "Scene:" line
          - per-step: action(target -> dest), affordance_region text, constraint
            categories, done_when. UV/approach coords are intentionally absent —
            runtime grounding (Lang-SAM) resolves affordance_region to image coords.
        """
        ...
        # <think>...</think> — causal reasoning leads, plan follows.
        reasoning = plan.get("reasoning", "")
        if isinstance(reasoning, str) and reasoning.strip():
            lines.append(f"<think>{reasoning.strip()}</think>")

        # Goal: sole termination criterion, evaluated continuously by MPC.
        goal = plan.get("goal", [])
        if isinstance(goal, list) and goal:
            ...
            for g in goal:
                if not isinstance(g, dict):
                    continue
                pred = g.get("pred", "")
                args = ", ".join(str(a) for a in g.get("args", []))
                if pred:
                    goal_parts.append(f"{pred}({args})")
            if goal_parts:
                lines.append(f"Goal: {'; '.join(goal_parts)}")
        ...
        for step in plan.get("steps", []):
            ...
            # affordance_region: textual description; Lang-SAM grounds at runtime.
            region = step.get("affordance_region", "")
            if isinstance(region, str) and region.strip():
                lines.append(f"  affordance_region: {region.strip()}")

            # Constraint categories. Approach axis now lives under
            # constraints.direction.approach_axis — no separate approach line.
